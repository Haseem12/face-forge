...
export default function FleexPage() {
  ...
  useEffect(() => {
    const getUser = async () => {
      ...
      await fetchFleex(Array.from(followingIds))
    }
    getUser()
  }, [supabase, router])

  const fetchFleex = async (followingIds: string[], refresh = false) => {
    ...
    try {
      ...
      // Step 1: Fetch fleex from followed users (without trying to join)
      ...
      if (!fleexData || fleexData.length === 0) {
        setFleex([])
        setLoading(false)
        return
      }
      
      // Step 2: Get unique user IDs from the fleex
      const uniqueUserIds = [...new Set(fleexData.map(f => f.user_id))]
      ...
      const mergedFleex = fleexData.map(f => ({
        ...f,
      ...
      // Step 6: Get user's likes and saves for these fleex
      if (currentUser) {
        const fleexIds = mergedFleex.map(f => f.id)
        if (fleexIds.length) {
